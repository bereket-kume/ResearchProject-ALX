...
const Dashboard = () => {
    ...
    const handleAddItem = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/api/items/create/', formData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('access')}`,
    ...
    const handleDeleteItem = async (itemId) => {
        try {
            await axios.delete(`http://localhost:8000/api/items/${itemId}/delete/`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('access')}`
                }
            });
    ...
    const handleUpdateItem = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:8000/api/items/${currentItem.id}/update/`, formData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('access')}`,
                    'Content-Type': 'multipart/form-data'
                },
